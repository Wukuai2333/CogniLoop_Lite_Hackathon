...
The hosted demo can run without collecting API keys. Full AI features are designed for local use through `.env`, where users bring their own key. This keeps the public demo safer while still showing how a local Cognee memory flow could work.

### Beginner-friendly RAG walkthrough

The project gives participants a minimal pattern they can understand and test:
...
"This is the overview page. It explains that CogniLoop Lite has two modes: a hosted guide that works without an API key, and a local full demo where participants clone the project and add their own `.env` to test Cognee."
...
CogniLoop Lite is a learning companion and a local demo workspace for testing Cognee memory.
...
This is a minimal Cognee memory flow: add documents, cognify them, recall answers.
...
Purpose: Help participants adapt the local demo.
...
The project shows how participants can replace the sample materials and test Cognee with their own local documents.
...
- Participants can use a local-first demo to test document memory and recall.
